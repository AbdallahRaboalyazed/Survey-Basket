    ...
    IEmailSender emailSender) : INotificationService
{
    ...
    public async Task SendNewPollsNotification(int? pollId = null)
    {
        ...
        if (pollId.HasValue)
        {
            var poll = await _context.Polls.SingleOrDefaultAsync(x => x.Id == pollId && x.IsPuplished);
        ...
        foreach (var poll in polls)
        {
            foreach (var user in users)
            {
                ...
                var body = EmailBodyBuilder.GenearteEmailbody("PollNotification", placeholders);
